...
import type { SleepStoriesAnnouncement } from "@shared/schema";
...
export default function SleepStoriesAdminAnnouncements() {
  const { toast } = useToast();
  const [editingId, setEditingId] = useState<string | null>(null);

  const { data: announcements, isLoading } = useQuery<SleepStoriesAnnouncement[]>({
    queryKey: ["/api/sleepstories/admin/announcements"],
  });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: AnnouncementFormValues) => {
      ...
      return apiRequest("POST", "/api/sleepstories/admin/announcements", payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/sleepstories/admin/announcements"] });
      queryClient.invalidateQueries({ queryKey: ["/api/sleepstories/announcements"] });
      form.reset();
      toast({
        title: "Success",
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: AnnouncementFormValues }) => {
      ...
      return apiRequest("PUT", `/api/sleepstories/admin/announcements/${id}`, payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/sleepstories/admin/announcements"] });
      queryClient.invalidateQueries({ queryKey: ["/api/sleepstories/announcements"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      return apiRequest("DELETE", `/api/sleepstories/admin/announcements/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/sleepstories/admin/announcements"] });
      queryClient.invalidateQueries({ queryKey: ["/api/sleepstories/announcements"] });
  ...
  const handleEdit = (announcement: SleepStoriesAnnouncement) => {
  ...
  return (
    <div className="p-6 md:p-8 space-y-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl md:text-4xl font-semibold">SleepStories Announcements</h1>
          <p className="text-muted-foreground">
            Create and manage announcements for SleepStories
          </p>
        </div>
      </div>
