...
import type { TrusttransportAnnouncement } from "@shared/schema";
...
export default function TrustTransportAdminAnnouncements() {
  const { toast } = useToast();
  const [editingId, setEditingId] = useState<string | null>(null);

  const { data: announcements, isLoading } = useQuery<TrusttransportAnnouncement[]>({
    queryKey: ["/api/trusttransport/admin/announcements"],
  });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: AnnouncementFormValues) => {
      ...
      return apiRequest("POST", "/api/trusttransport/admin/announcements", payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/trusttransport/admin/announcements"] });
      queryClient.invalidateQueries({ queryKey: ["/api/trusttransport/announcements"] });
      form.reset();
      toast({
        title: "Success",
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: AnnouncementFormValues }) => {
      ...
      return apiRequest("PUT", `/api/trusttransport/admin/announcements/${id}`, payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/trusttransport/admin/announcements"] });
      queryClient.invalidateQueries({ queryKey: ["/api/trusttransport/announcements"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      return apiRequest("DELETE", `/api/trusttransport/admin/announcements/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/trusttransport/admin/announcements"] });
      queryClient.invalidateQueries({ queryKey: ["/api/trusttransport/announcements"] });
  ...
  const handleEdit = (announcement: TrusttransportAnnouncement) => {
  ...
  return (
    <div className="p-6 md:p-8 space-y-8">
      <div className="flex items-center gap-4">
        <Link href="/apps/trusttransport/admin">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Create and manage TrustTransport announcements
          </p>
        </div>
      </div>
