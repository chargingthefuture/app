...
import type { SocketrelayAnnouncement } from "@shared/schema";
...
export default function SocketRelayAdminAnnouncements() {
  const { toast } = useToast();
  const [editingId, setEditingId] = useState<string | null>(null);

  const { data: announcements, isLoading } = useQuery<SocketrelayAnnouncement[]>({
    queryKey: ["/api/socketrelay/admin/announcements"],
  });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: AnnouncementFormValues) => {
      ...
      return apiRequest("POST", "/api/socketrelay/admin/announcements", payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/socketrelay/admin/announcements"] });
      queryClient.invalidateQueries({ queryKey: ["/api/socketrelay/announcements"] });
      form.reset();
      toast({
        title: "Success",
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: AnnouncementFormValues }) => {
      ...
      return apiRequest("PUT", `/api/socketrelay/admin/announcements/${id}`, payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/socketrelay/admin/announcements"] });
      queryClient.invalidateQueries({ queryKey: ["/api/socketrelay/announcements"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      return apiRequest("DELETE", `/api/socketrelay/admin/announcements/${id}`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/socketrelay/admin/announcements"] });
      queryClient.invalidateQueries({ queryKey: ["/api/socketrelay/announcements"] });
  ...
  const handleEdit = (announcement: SocketrelayAnnouncement) => {
  ...
  return (
    <div className="p-6 md:p-8 space-y-8">
      <div className="flex items-center gap-4">
        <Link href="/apps/socketrelay/admin">
          <Button variant="ghost" size="icon" data-testid="button-back">
            <ArrowLeft className="w-5 h-5" />
          </Button>
        </Link>
        <div>
          <h1 className="text-3xl md:text-4xl font-semibold">SocketRelay Announcements</h1>
